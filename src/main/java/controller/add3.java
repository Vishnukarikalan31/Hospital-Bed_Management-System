...
public class add3 extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(addpatientdao.add(patient)) {
		    ...
		    response.getWriter().write("<script> alert('Registration Successful!'); window.location.replace('vpicu.jsp'); </script>");
		}
		else {
			response.setContentType("text/html;charset=UTF-8");
		    response.getWriter().write("<script> alert('Registration Unsuccessful!'); window.location.replace('vpicu.jsp'); </script>");
		}
	}
