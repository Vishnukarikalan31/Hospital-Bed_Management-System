...
/**
 * Servlet implementation class add4
 */
@WebServlet("/add4")
	...
	private static final long serialVersionUID = 1L;
		...
		protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
			...
			if(addpatientdao.add(patient)) {
			    ...
			    response.getWriter().write("<script> alert('Registration Successful!'); window.location.replace('ViewPatient.jsp'); </script>");
			}
			else {
				response.setContentType("text/html;charset=UTF-8");
			    response.getWriter().write("<script> alert('Registration Unsuccessful!'); window.location.replace('ViewPatient.jsp'); </script>");
			}
		}
