...
/**
 * Servlet implementation class ICUServlet
 */
@WebServlet("/icuServlet")
public class ICUServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 String bedno = request.getParameter("bedno");
	        ...
	        if(bedDAO.addICU(bed)) {
				response.setContentType("text/html;charset=UTF-8");
			    response.getWriter().write("<script> alert('Bed Added Successful!'); window.location.replace('AdminDashBoard.jsp'); </script>");
			}
			else {
				response.setContentType("text/html;charset=UTF-8");
