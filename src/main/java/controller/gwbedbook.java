...
@WebServlet("/gwBedBook")
public class gwbedbook extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            try (Connection con = DBUtil.getConnection();
                 PreparedStatement preparedStatement = con.prepareStatement("UPDATE generalwardbed SET patientid=? WHERE id=?")) {

                preparedStatement.setLong(1, patientId);
                preparedStatement.setInt(2, bedId);
                preparedStatement.executeUpdate();

                String alertScript = "<script>alert('Successfully booked'); window.location.href='NurseGeneralWard.jsp';</script>";
            ...
            String alertScript = "<script>alert('Invalid Bed ID'); window.location.href='NurseGeneralWard.jsp';</script>";
            response.getWriter().write(alertScript);
        }
    }
}
